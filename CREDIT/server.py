...
"""
   Used to normalise the given values for generation of chart
"""
def normalise(environ,start_response):
...
"""
   Adds the entered series data into database 
"""
...
"""
  Used to make application offline
"""
def manifest(environ, start_response):
    start_response('200 OK', [('content-type','text/cache.manifest')])
    values = {}

    page = templating.load_page(values, "cache.manifest")
    return [page,] 

"""
   Used to check the connectivity of the application
"""
def isOnline(environ,start_response):
...
"""
   web service which returns the json data based on the provided url
"""
